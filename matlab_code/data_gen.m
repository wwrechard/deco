function [X, Y, beta] = data_gen(n, p, s, type, R2, r)
% Function to generate the experiment data for testing DECO and other
% regularized regression methods. The noise follows Gaussian distribution.
%
% Args:
%   n: Sample size.
%   p: Number of variables.
%   s: Number of non-zero coefficients. (Not used if type == 'l1-ball')
%   type: The type of correlation structure of X. Possible options are
%         'ind': independent variables.
%         'corr': correlated variables.
%         'group': grouped correlated variables.
%         'factor': latent factor models
%         'l1-ball': the strcuture is similar to 'corr' but beta is drawn
%         from an l1-ball.
%   R2: The R square. Determiniing the signal-to-noise ratio.
%   r: Used for type 'corr', 'group' and 'factor'. if type == 'corr', r is
%   the correlation. if type == 'group', r is the number of groups. if type
%   == 'factor', r is the number of latent factors.
%
% Returns:
%   A list of generated data including X, Y and the true coefficient beta.
%
% Example:
%   [X, Y, beta] = data_gen(100, 1000, 5, 'corr', 0.9, 0.5);
    if nargin == 2
        s = 5;
        type = 'ind';
        R2 = 0.95;
